<template>
<div class="vue-upload-file" :class="{'large-img-file': imgType === 'large', 'small-img-file': imgType === 'small'}" :style="{height: shapeSize, width: shapeSize}">
	<div class="vuf-drop-area" 
		:style="{height: shapeSize, width: shapeSize}"
		@click="handleClick"
...
export default {
	...
	methods: {
		...
		upload(files){
			let that = this,
				{url, params, onlySingle, field, size, lang} = this,
				fmData = new FormData();
				console.log(field);
			// 判断是否单文件
			if(onlySingle){
			...
			new Promise(function (resolve, reject) {
				...
				function (resData) {
					that.loading = 2;
					that.$emit('uploadSuccess', resData, field, size);
				},
				// 上传失败
				function (sts) {
					that.loading = 3;
					...
					that.$emit('uploadFail', sts, field, size);
				}
			);
		}
	}
